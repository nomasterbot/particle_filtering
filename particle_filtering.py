...
def landmark_trans(landmarks):
    flag = 1;
    for i,landmark in enumerate(landmarks):
        landmarks[i][1] = (landmark[1]+flag*5)%HEIGHT
        flag = -flag
...
while(1):
    ...
    #draw_expected_position:
    est_particle = estimate(particles, weights)[0]
    #print(est_particle)
    cv2.circle(img, tuple((int(est_particle[0]),int(est_particle[1]))),2,(0,0,255),-1)
